...
$_MODULE_PARAM = array(
	"name"			=> "item_size",
	"tableName" 	=> $_VARS['tbl_prefix']."_catalog_size",
	"userAccess" 	=> array("admin", "editor")	
);

$_TEXT = array(
	"TEXT_HEAD"		=> "Габариты",
	"TEXT_ADD_ITEM"	=> "Добавить габарит",
	"TEXT_EDIT_ITEM"=> "Редактировать габарит"		
);
/*~~~~~~~~~~~~~~~~~~~*/
/* /параметры модуля */
/*~~~~~~~~~~~~~~~~~~~*/
...
$arrTableFields = array(
	...
	"size_name"		=> "text", 									// обозначение
	"size_weight"	=> "decimal(11,2) default '0.00' not null", 	// масса для этого размера
	"size_descr"	=> "text", 									// описание 
	"size_shape"	=> "text", 									// форма 
	"size_order"	=> "int default '0' not null", 				// сортировка
	"size_show"		=> "enum('1', '0') not null"				// показывать
);
/*~~~~~~~~~~~~~~~~~~~~~~~*/
/* /структура таблицы БД */
/*~~~~~~~~~~~~~~~~~~~~~~~*/
...
if(isset($addItem))
{
	...
	$arrData["size_order"] = mysql_insert_id();
...
{
	$db_Table -> tableOrderField = "size_order";
...
if(!isset($editItem) && !isset($setItem))
{
	...
	<fieldset><legend><?=$_TEXT['TEXT_HEAD']?></legend>
		...
		<?
		GetItems($_MODULE_PARAM['tableName'], $orderBy = "size_order", $orderDir = "ASC");
		?>
		<a class="serviceLink" href="<?=$_SERVER['REQUEST_URI'];?>&setItem"><img src='<?=$_ICON["add_item"]?>'><?=$_TEXT['TEXT_ADD_ITEM']?></a>
	</fieldset>
	<?
