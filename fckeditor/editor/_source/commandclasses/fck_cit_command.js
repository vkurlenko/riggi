...
FCKBlockQuoteCommand.prototype =
{
	Execute : function()
	{
		...
		if ( FCKBrowserInfo.IsIE )
		{
			...
			if ( bStart
					&& bStart.parentNode.nodeName.IEquals( '<cit>' )
			...
			if ( bEnd
					&& bEnd.parentNode.nodeName.IEquals( '</cit>' )
		...
		if ( state == FCK_TRISTATE_OFF )
		{
			...
			var bqBlock = range.Window.document.createElement( 'cit' ) ;
		...
		else if ( state == FCK_TRISTATE_ON )
		{
			...
			while ( ( block = iterator.GetNextParagraph() ) )
			{
				...
				{
					if ( block.parentNode.nodeName.IEquals( 'cit' ) )
					{
	...
	GetState : function()
	{
		...
		{
			if ( path.Elements[i].nodeName.IEquals( 'cit' ) )
